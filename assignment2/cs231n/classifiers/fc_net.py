...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        # OLD implementation:
        # Works fine for 2-layer FullyConnectedNet
        # the cached outputs list containing tuples of caches (1 per computation)
        # is _extremely_ ugly when used for anything more complicated.
        #
        # actually I don't even really like it in the 2-layer net,
        # since the affine cached outputs has a longer length than all other
        # cached output lists.
        #
        # it's just obfuscating the intent of what you're doing.
        pass
        # ############################################################################
        ...
        # fc_relu_caches = [*np.zeros(self.num_layers)]
        # dropout_caches = [*np.zeros(self.num_layers - 1)]
        # scores, fc_relu_caches[0] = affine_relu_forward(
        #     X, self.params["W1"], self.params["b1"]
        # )
        #
        # for i in range(1, self.num_layers - 1):
        #     scores, fc_relu_caches[i] = affine_relu_forward(
        #         scores, self.params[f"W{i + 1}"], self.params[f"b{i + 1}"]
        #     )
        #
        # scores, fc_relu_caches[-1] = affine_forward(
        ...
        # # I can't remember why, but I get all weight
        # regularizer_sum = 0
        # for i in range(self.num_layers):
        ...
        # for i, cache in enumerate(reversed(fc_relu_caches)):
        #     if i == 0:
        #         dXs[i], dWs[i], dbs[i] = affine_backward(d_softmax, cache)
        #     else:
        #         dXs[i], dWs[i], dbs[i] = affine_relu_backward(dXs[i - 1], cache)
        # dXs.reverse()
        # dWs.reverse()
        # dbs.reverse()
        #
